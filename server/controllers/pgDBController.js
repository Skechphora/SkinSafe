...
for (let PRODUCT of res.locals.CATEGORY) {
  ...
  for (let {ingredient, display_name, variant} of PRODUCT.sub_product_list) {
    ...
    const obj = {display_name: display_name,
      variant: variant,
      ingredient:[]}
...
saveSub_product: async (req,res,next)=> {
  ...
  for (let PRODUCT of res.locals.SORTED_CATEGORY) {
    ...
    for (let obj of sub_product_list) {
      console.log(obj)
      const result = await pgSql.query(`INSERT INTO sub_product (sub_product_id, display_name, hero_image, rating, reviews, product_id, variant) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING`,
      [product_id+'::'+obj.display_name+obj.variant, obj.display_name, hero_image, rating, reviews, product_id, obj.variant])
      console.log(result)
    }
  }
  return next()
      ...
      for (let obj of sub_product_list) {
        ...
        for (let ele of obj.ingredient) {
          ...
          if (product_id && obj.display_name) {
            ...
            const jointResult = await pgSql.query(commandJoint, [ product_id+"::"+ obj.display_name+obj.variant, ingredient ])
...
getAllsubProduct: async (req,res,next) =>{
  try {
    ...
    for(let SUB_PRODUCT of res.locals.SUB_PRODUCT_LIST) {
      const {sub_product_id, display_name, hero_image, rating, reviews} = SUB_PRODUCT
      console.log('getting all the subProdut with ingredients')
      const QUERY = "SELECT pi.sub_product_id, i.ingredient, i.description FROM product_ingredient pi LEFT JOIN ingredient_list i ON i._id=pi.ingredient_id WHERE pi.sub_product_id =$1;"
