...
const FilterComponent = () => {
  	...
  	// populates an array of one list item per allergen to render 
		...
		for (let [i, product] of productList.entries()) {
				if (!brandCache[product['brand_name']]) {
						brands.push(<option key={i} value={product['brand_name']}>{product['brand_name']}</option>)
						brandCache[product['brand_name']] = true;
				}
				if (!categoryCache[product['category']]) {
					categories.push(<option key={i} value={product['category']}>{product['category']}</option>)
  ...
  return (
    <>
			<form id="filter-form" onSubmit={() => clickHandler()}>
				<label htmlFor="brand-filter">Filter by Brand:</label>
				<select className="brandFilterBox" id="brandFilterBox" placeholder="Select Brand"> 
					<option value={brand} ref={selectedBrand}>Select Brand</option>
					{brands}
				</select>
				<label htmlFor="category-filter">Filter by Category: </label>
				<select className="categoryFilterBox" id="categoryFilterBox" placeholder="Select Category">
					<option value={category} ref={selectedCategory}>Select Category</option>
					{categories}
				</select>
				<button id="filter-button" type="submit" onClick={e => e.preventDefault()}>Filter Results</button>
			</form>
    </>
  );
};
